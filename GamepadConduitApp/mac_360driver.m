...
static void X360ValueCallback(void* context, IOReturn ior, void* userData, IOHIDValueRef value) {
	...
	if (usage == 50) {
		...
		if (normValue < 0) {
			normValue = fminf(0.0f, normValue + deadZone) / (32768.f - deadZone);
		}
		else {
			normValue = fmaxf(0.0f, normValue - deadZone) / (32767.f - deadZone);
		}
		...
		switch (usage) {
			...
			case 11: button = &controller->menu; break;
...
@implementation X360ControllerDriver
